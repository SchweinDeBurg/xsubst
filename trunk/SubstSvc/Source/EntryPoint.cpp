...
int _tmain(int argc, TCHAR* argv[], TCHAR* /*envp*/[])
{
	...
	LogFile_WriteEntry(LL_MINIMAL, _T("Started %s executable."), argv[0]);
	...
	if (argc > 1) {
		...
		if (::lstrcmpi(argv[1], _T("/Register")) == 0) {
			...
			LogFile_WriteEntry(LL_MINIMAL, _T("Registered %s service."), g_szServiceName);
		}
			...
			::CloseServiceHandle(schService);
			LogFile_WriteEntry(LL_MINIMAL, _T("Unregistered %s service."), g_szServiceName);
		}
		else {
			// unrecognized command-line argument
			LogFile_WriteEntry(LL_MINIMAL, _T("Command-line argument %s unrecognized."), argv[1]);
		}
		...
		LogFile_WriteEntry(LL_MINIMAL, _T("Attempted to connect to the SCM."));
		::StartServiceCtrlDispatcher(aste);
		LogFile_WriteEntry(LL_MINIMAL, _T("Disconnected from the SCM."));
	}
...
#endif	// _DEBUG
	LogFile_WriteEntry(LL_MINIMAL, _T("Finished %s executable.\n<<< end of log"), argv[0]);
